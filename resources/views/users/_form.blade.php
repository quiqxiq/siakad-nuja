<div x-data="{ role: '{{ old('role', $user->role ?? 'guru') }}' }" class="space-y-6">
    <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
        ...
        <x-form.input label="Nomor HP / WhatsApp" name="no_hp" type="tel" inputmode="numeric" pattern="[0-9+\s-]*" :value="$user->no_hp ?? ''" placeholder="08..." hint="Nomor WhatsApp untuk penerimaan kredensial akun" />

        {{-- Field khusus guru --}}
        <template x-if="role === 'guru'">
            <div class="contents">
                <x-form.input label="NIP" name="nip" type="number" min="1" inputmode="numeric" class="no-spinner" onkeydown="if(['e','E','+','-','.'].includes(event.key)) event.preventDefault()" :value="$user->guru->nip ?? ''" hint="Wajib diisi untuk guru." />
